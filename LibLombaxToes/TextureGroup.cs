namespace LibLombaxToes
{
	public class TextureGroup
	{
		...
		public byte[] RipTexture(ulong tuid, bool ripMipmaps, out TextureFormat format, out int width, out int height, out int mipmapCount)
		{
			...
			width = textures[index].width / (highmipsStream == null ? 2 : 1);
			height = textures[index].height / (highmipsStream == null ? 2 : 1);
			mipmapCount = 1;
		...
		public uint CalculateTextureSize(ulong tuid, int mipmap)
		{
			...
			switch(textures[index].format)
			{
				...
				case TextureFormat.DXT5:
					uint size = 0;
					for(int i = (mipmap < 0 ? 0 : mipmap) - (highmipsStream == null ? 1 : 0); i < (mipmap < 0 ? textures[index].mipmapCount : mipmap + 1); i++)
					{
						size += (uint)(
