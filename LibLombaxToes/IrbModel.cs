	...
	public class IrbModel : IGHW
	{
		...
		void ReadMeshScale()
		{
			if(type == 0)
			{
				...
				meshScaleX = meshScaleY = meshScaleZ = BitConverter.ToSingle(BitConverter.GetBytes(sh.ReadUInt32() + 0x08000000));
			}
			else if (type == 1)
			{
				...
				meshScaleX = sh.ReadSingle() * (float)Math.Pow(2, 16);
				meshScaleY = sh.ReadSingle() * (float)Math.Pow(2, 16);
				meshScaleZ = sh.ReadSingle() * (float)Math.Pow(2, 16);
			}
		}
